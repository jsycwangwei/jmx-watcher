...
public class JmxResultProcessor {
    ...
    public JmxResultProcessor(Query query, ObjectInstance objectInstance, List<Attribute> attributes,
            String className) {
        this.query = query;
    ...
    /**
     * Populates the Result objects. This is a recursive function. Query contains the keys that we want to get the
     * values of.
     */
    ...
    private void processTabularDataSupport(Builder<Result> accumulator, String attributeName, TabularDataSupport tds) {
        ...
        for (Map.Entry<Object, Object> entry : entries) {
            ...
            if (entryKeys instanceof List) {
                ...
                if (entryValue instanceof CompositeDataSupport) {
                    ...
                    throw new RuntimeException("!!!!!!!!!! Please file a bug: JMX-WATCHER entryValue is: "
                            + entryValue.getClass().getCanonicalName());
                }
            }
            else {
                throw new RuntimeException("!!!!!!!!!! Please file a bug: JMX-WATCHER entryKeys is: "
    ...
    private Result getNewResultObject(String attributeName, Map<String, Object> values) {
        return new Result(System.currentTimeMillis(), attributeName, className, query.getResultAlias(),
                objectInstance.getObjectName().getCanonicalKeyPropertyListString(), values);
    }
}
